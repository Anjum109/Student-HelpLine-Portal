...
<?php 	
																...
																$fetchqry2 = "UPDATE `html_quiz` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
if(isset($c)) {    $records = mysqli_query($dbc,"select * from `html_quiz` where id='$c'");
...
<tr><td><h3><br><?php echo @$row['que'];?></h3></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 5){ ?>
 ...
 if($_SESSION['clicks']>4){ 
	...
	$qry3 = "SELECT `ans`, `userans` FROM `html_quiz`where cat_name='html';";
 ...
 <a style="color:red; text-decoration:none;"href="phpquiz_answer.php">See Answer</a>
<?php } ?>
