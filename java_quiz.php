...
<?php 	
                                ...
                                { $userselected = $_POST['userans'];
																...
																$fetchqry2 = "UPDATE `java_quiz` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
if(isset($c)) {   
  ...
  $records = mysqli_query($dbc,"select * from `java_quiz` where id='$c' AND cat_name='java'");
...
<tr><td><h3><br><?php echo @$row['que'];?></h3></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 5){ ?>
 ...
 if($_SESSION['clicks']>4){ 
	...
	$qry3 = "SELECT `ans`, `userans` FROM `java_quiz`where cat_name='java';";
 ...
 <a style="color:red; text-decoration:none;"href="javaquiz_answer.php">See Answer</a>
<?php } ?>
